...
Both read the service-account key from that repository secret. The old GitHub Pages address serves a "moved" page — `move/index.html` and `move/main.js`, built by `npm run build:move` into a single file in `dist-move/` and published to `gh-pages` once by `deploy.yml` — which carries a visitor's picks, verdicts and notes to the new origin. It shares the codec with the app by importing `src/core/notes.js`, so the link it makes is one the live site understands.
