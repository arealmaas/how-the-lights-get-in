...
- `tests/e2e/` — Playwright; `firebase/test/` — the Firestore rules suite (needs a JDK).

## Deployment and pull-request previews

Merging to `main` is the deploy: `firebase-hosting-merge.yml` publishes the site and the rules, and `deploy.yml` (one last run) replaces the old GitHub Pages site with the move page. So before the hosting branch merges, the Firebase project must exist with Firestore created and **Hosting started** (console → Build → Hosting → *Get started*; without it the deploy action has no site to publish to — `CREW-SPEC.md` section 9, step 4), the repository secret `FIREBASE_SERVICE_ACCOUNT_HOW_THE_LIGHT_GETS_IN` must be set (created by `firebase init hosting:github`), and its service account must hold the *Firebase Rules Admin* role. Once `deploy.yml` has run its one job and the old address is serving the move page, delete `deploy.yml`: it publishes to `gh-pages` on every push to `main` and would otherwise keep rebuilding a page that never changes. The project id `how-the-light-gets-in` appears in `README.md`, `ROADMAP.md`, `.firebaserc`, `src/data/index.js` (`PUBLIC_URL`), `move/main.js` and both `firebase-hosting-*.yml`; a different id is a search-and-replace across those seven files, plus `src/data/index.test.js` and `src/cloud/crew.test.js`, which assert the URL and fail until you do.
...
Both need the repository secret created by `firebase init hosting:github`, and the deploy service account needs the *Firebase Rules Admin* role. The old GitHub Pages address serves a "moved" page — `move/index.html` and `move/main.js`, built by `npm run build:move` into a single file in `dist-move/` and published to `gh-pages` once by `deploy.yml` — which carries a visitor's picks, verdicts and notes to the new origin. It shares the codec with the app by importing `src/core/notes.js`, so the link it makes is one the live site understands.
