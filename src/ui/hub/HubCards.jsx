...
// calendar export of all picks, the share link (picksLink, with a Copy button) and the notes/verdicts
// export. `mine` is the picked events, computed once by HubSheet.
...
import {notesMarkdown, picksLink} from '../../core/exports.js';
...
export default function HubCards({mine}){
  ...
  const noteN = Object.keys(notes).filter(k => notes[k] && notes[k].trim()).length;
  ...
  function exportNotes(){
    download('htlgi-london-2026-notes.md', notesMarkdown(EVENTS, picks, notes, verdicts), 'text/markdown;charset=utf-8');
  }
  ...
  return (
    <div className="hub-cards">
      ...
      <div className="hub-card">
        <span className="hc-k">Notes</span>
        <span className="hc-d">
          {noteN || verdictN
            ? `${noteN} note${noteN === 1 ? '' : 's'} · ${verdictN} verdict${verdictN === 1 ? '' : 's'}`
            : 'No notes yet — every event sheet has a notes box, debates have a “who won?” vote.'}
        </span>
        {(noteN || verdictN) > 0 && (
          <div className="actions">
            <button type="button" className="btn" onClick={exportNotes}>Export notes (.md)</button>
          </div>
        )}
      </div>
    </div>
