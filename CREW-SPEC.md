...
**Codebase:** see `docs/superpowers/specs/2026-09-07-react-restructure-design.md`. The app is a Vite 8 + React 19 codebase (`src/`), built with `npm run build` into `dist/`; the module list in this section, which describes the single-file `scripts/template.html` this spec was written against, is superseded by that document. The behaviour in sections 1–7 is unchanged.
...
- `firebase.json` at the repo root: hosting (`public: dist`; `Cache-Control: no-cache` for `/`, `/index.html`, `/sw.js` and `/manifest.webmanifest`; `public, max-age=31536000, immutable` for `/img/**` and for the hashed bundles under `/assets/**`), `firestore.rules: firebase/firestore.rules`, emulator ports. `.firebaserc` with the project id.
- Workflows: `firebase init hosting:github` writes `firebase-hosting-pull-request.yml` (a preview channel and a PR comment for every pull request, expiring after seven days) and `firebase-hosting-merge.yml` (live deploy on push to `main`), and stores a service-account secret. Build step in both: `npm ci` then `npm run build`, with `VITE_PREVIEW: "PR #${{ github.event.number }}"` on the PR workflow's build step so previews keep their ribbon and `noindex` (`src/App.jsx`). The merge workflow runs `npm test` and the rules tests first, on an explicit temurin JDK 21 (`actions/setup-java`) because the emulator is a Java process. Add to the merge workflow a rules deploy (`npx firebase-tools deploy --only firestore:rules`) with the same secret; the service account needs the *Firebase Rules Admin* role for that. Preview channels use the real Firestore project, as PR previews do today. They are separate origins, so they cannot evict the live site's caches; Google sign-in on a preview would need its domain authorized, so test sign-in on `localhost` and the live site. Delete `deploy.yml` once the move page is up; the two `firebase-hosting-*.yml` files are the only workflows that stay (an earlier draft of this spec named a `pr-preview.yml`, which was never written — `firebase-hosting-pull-request.yml` is the preview workflow).
...
- URL constants: `PUBLIC_URL` in `src/data/index.js`, the links in the README, `NEW` in `move/main.js`. `manifest.webmanifest` is relative and needs no change. Keep the `.ics` `UID` suffix `@arealmaas.github.io`: it is a namespace, and changing it would duplicate events for anyone who re-imports a calendar.
- `data/firebase.json` (public web config from the console: `apiKey`, `authDomain`, `projectId`, `appId`) → read by `src/data/index.js` with `import.meta.glob` and bundled as `FIREBASE`; when absent, `CLOUD` is false and the Account and Crew UI are hidden. It is public by design; the rules are the security.
...
- `src/sw.js` (the service-worker source `vite-plugin-pwa` injects the precache list into): no gstatic host is needed — the SDK is an npm dependency bundled into its own chunk and served from this origin, not fetched from `https://www.gstatic.com/firebasejs/`. Never cache `/__/*` (the auth helpers are same-origin now) or `apis.google.com`; the cache is named `htlgi-<scope hash>-<build>` and the sweep drops only caches with the same scope prefix, so two copies of the app on one origin cannot evict each other. Photos live in `htlgi-img-v1`, which the sweep spares: a photo replaced without a change of filename needs that suffix bumped.
- The app: an account layer (auth, sign-in sequence, merge, own-state sync, Account card) and a crew layer (crews, invites, removal, overlay, hub section, crew calendar, reading-list toggle, join banner, `#join=` and `notes=` handling). These are `src/cloud/auth.js`, `src/cloud/sync.js`, `src/cloud/crew.js`, `src/routing.js` and the components under `src/ui/hub/` and `src/ui/event/`; see the design document named at the top of this section.
...
6. On your machine: `npm i -g firebase-tools`, `firebase login`, then from the repo root `firebase init hosting` (public directory `dist`, not a single-page app, GitHub deploys yes) and `firebase init hosting:github` (build script `npm ci && npm run build`, deploy on merge yes). Both are already in the repo, so this is a check rather than a rewrite: keep `firebase.json`'s headers and the rules deploy step in `firebase-hosting-merge.yml`. Commit the two workflows, `firebase.json` and `.firebaserc`. IAM → grant the new service account *Firebase Rules Admin*.
